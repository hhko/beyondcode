...
------------------------------- |---------------------------------------------------|----------------------------------
변경에 강한 모델                                 | 복잡성 분리                      | 부작용 최소화 (부작용 없는 순수 함수 구성)
예측 가능한 동작 (명확한 책임 분리, 테스트 용이성) | 명확한 경계 (Bounded Context)    | 순수 함수 중심 (상태 변화는 명시적 함수 결과로 표현)
정확한 도메인 표현 (도메인 언어와 코드가 일치)     | 명확한 의미 부여 (유비쿼터스 언어) | 타입 기반 설계
