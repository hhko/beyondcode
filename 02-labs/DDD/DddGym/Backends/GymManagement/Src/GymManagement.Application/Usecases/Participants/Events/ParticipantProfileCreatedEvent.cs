...
public static class ParticipantProfileCreatedEvent
{
        ...
        : IDomainEventUsecase<UserEvents.ParticipantProfileCreatedEvent>
    {
        ...
        {
            var usecase = from participant in Participant.Create(userId: domainEvent.UserId, id: domainEvent.ParticipantId)
                          from _ in _participantsRepository.AddParticipantAsync(participant)
                          select unit;
