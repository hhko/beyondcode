...
**공통 목표(장점)** |  **DDD 가치**                    | **FP 가치**
------------------------------- |---------------------------------------------------|----------------------------------
변경에 강한 모델                                 | 복잡성 분리 <br/> (관심사의 분리)          | 부작용 최소화 <br/> (부작용 없는 순수 함수 구성)
예측 가능한 동작 <br/> (명확한 책임 분리, 테스트 용이성)  | 명확한 경계 <br/> (Bounded Context) | 순수 함수 중심 <br/> (상태 변화는 명시적 함수 결과로 표현)
정확한 도메인 표현 <br/>  (도메인 언어와 코드가 일치)     | 명확한 의미 <br/> (유비쿼터스 언어) | 타입 기반 설계
